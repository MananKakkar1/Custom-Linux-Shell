#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <stdio.h>

#include "io_helpers.h"
#include "variables.h"
#include "builtins.h"

void free_all_tokens(char **tokens) {
	for (size_t i = 0; tokens[i] != NULL; i++) {
		free(tokens[i]);
	}
}
char *expand_variables(const char *input) {
	char *expanded = strdup(input);
	char result[129] = "";
	if (expanded[0] == '$') {
		char *token = strtok(expanded, "$");
		while (token != NULL) {
			const char *value = get_variable(token);
			if (value != NULL) {
				strcat(result, value);
			}
			token = strtok(NULL, "$");
		}
	} else if (expanded[0] != '$') {
		char *token = strtok(expanded, "$");
		strcat(result, token);
		token = strtok(NULL, "$");
		while (token != NULL) {
			const char *value = get_variable(token);
			if (value != NULL) {
				strcat(result, value);
			}
			token = strtok(NULL, "$");
		}
	}
	free(expanded);
	return strdup(result);
}
// ===== Output helpers =====

/* Prereq: str is a NULL terminated string
 */
void display_message(char *str) {
    write(STDOUT_FILENO, str, strnlen(str, MAX_STR_LEN));
}


/* Prereq: pre_str, str are NULL terminated string
 */
void display_error(char *pre_str, char *str) {
    write(STDERR_FILENO, pre_str, strnlen(pre_str, MAX_STR_LEN));
    write(STDERR_FILENO, str, strnlen(str, MAX_STR_LEN));
    write(STDERR_FILENO, "\n", 1);
}


// ===== Input tokenizing =====

/* Prereq: in_ptr points to a character buffer of size > MAX_STR_LEN
 * Return: number of bytes read
 */
ssize_t get_input(char *in_ptr) {
    int retval = read(STDIN_FILENO, in_ptr, MAX_STR_LEN+1); // Not a sanitizer issue since in_ptr is allocated as MAX_STR_LEN+1
    int read_len = retval;
    if (retval == -1) {
        read_len = 0;
    }
    if (read_len > MAX_STR_LEN) {
        read_len = 0;
        retval = -1;
        write(STDERR_FILENO, "ERROR: input line too long\n", strlen("ERROR: input line too long\n"));
        int junk = 0;
        while((junk = getchar()) != EOF && junk != '\n');
    }
    in_ptr[read_len] = '\0';
    return retval;
}

/* Prereq: in_ptr is a string, tokens is of size >= len(in_ptr)
 * Warning: in_ptr is modified
 * Return: number of tokens.
 */
size_t tokenize_input(char *in_ptr, char **tokens) {
    // TODO: Remove unused attribute
    char *curr_ptr = strtok(in_ptr, DELIMITERS);
    size_t token_count = 0;
	size_t counter = 0;
    while (curr_ptr != NULL) {  // TODO: Fix this
        // TODO: Fix this
		tokens[token_count] = strdup(curr_ptr);
		curr_ptr = strtok(NULL, DELIMITERS);
		token_count += 1;
    }
    tokens[token_count] = NULL;
	size_t count = 0;
	while (tokens[count] != NULL) {
		if (contains_var(tokens[count]) == 1 && !(tokens[count][0] == '$' && tokens[count][1] == '\0')) {
			char *result = malloc(129);
			if (result != NULL) {
				result[0] = '\0';
			}
			if (tokens[count][0] == '$') {
				char *token = strtok(tokens[count], "$");
				while (token != NULL) {
					const char *value = get_variable(token);
					if (value != NULL) {
						char trunc[MAX_STR_LEN+1] = {0};
						int curr = 0;
						while (counter < MAX_STR_LEN && value[curr] != '\0') {
							trunc[curr] = value[curr];
							counter ++;
							curr++;
						}
						trunc[curr] = '\0';
						strcat(result, trunc);
					}
					token = strtok(NULL, "$");
				}
			} else {
				char *token = strtok(tokens[count], "$");
				if (token != NULL) {
					strcat(result, token);
				}
				token = strtok(NULL, "$");
				while (token != NULL) {
					const char *value = get_variable(token);
					if (value != NULL) {
						char trunc[MAX_STR_LEN+1] = {0};
						int curr = 0;
						while (counter < MAX_STR_LEN && value[curr] != '\0') {
							trunc[curr] = value[curr];
							counter ++;
							curr++;
						}
						trunc[curr] = '\0';
						strcat(result, trunc);
					}
					token = strtok(NULL, "$");
				}
			}
			free(tokens[count]);
			tokens[count] = strdup(result);
			free(result);
		}
		count++;
	}
    return token_count;
}
